...
class RequestHandler
{
    ...
    public static $file_metadata_name = '.metadata';

    // should transform file
    public static $file_store_function = array('FilePond\\RequestHandler', 'storeTempFile');
    ...
    private static function loadFileObjects($fieldName) {
        ...
        $DATA = isset($_POST[$fieldName]) ? $_POST[$fieldName][0] : null;

        if (is_array($FILE['tmp_name'])) {

            foreach( $FILE['tmp_name'] as $index => $tmpName ) {

                $item = self::createItem( array(
    ...
    private static function loadBase64FormattedFiles($fieldName) {
        ...
        */
        ...
        foreach ($values as $value) {
            ...
            // create the item and set its metadata
            $item = self::createItem(self::createTempFile($obj));
    ...
    private static function loadFilesFromTemp($fieldName) {
        ...
        foreach ($values as $value) {
            if ( self::isFileId($value) ) {
                $item = self::createItemFromTempFileById($value);
                array_push($items, $item);
            }
        }
    ...
    private static function createItemFromTempFileById($id) {
        $file = self::getTempFile($id);
        $item = self::createItem($file, $id);
        $data = self::getTempFileData($id);
    ...
    private static function saveTempFile($file) {
        ...
        if (isset($metadata)) {
            $handle = fopen($path . self::$file_metadata_name, 'w');
            fwrite($handle, @json_encode($metadata));
            fclose($handle);
        }
    ...
    public static function getTempFileData($fileId) {

        // select all files in directory except .htaccess
        foreach(glob(self::getSecureTempPath() . $fileId . DIRECTORY_SEPARATOR . self::$file_metadata_name) as $file) {
    ...
    private static function saveFile($item, $path) {
        ...
        // if is file id
        if (is_string($item)) {
            // get the item and then call this method again
            return self::saveFile(self::createItemFromTempFileById($item), $path);
        }

        // is file item object
        return self::moveTempFile($item, $path);
    }

    private static function moveTempFile($item, $path) {
        ...
        foreach(glob(self::getSecureTempPath() . $fileId . DIRECTORY_SEPARATOR . '*.*') as $file) {
            ...
            // copy the file from source to target, this allows setting a custom handler
            call_user_func(
                self::$file_store_function,
                $item, 
                $source, 
                $target . (isset($fileName) ? basename($fileName) : basename($file))
            );
        }
    ...
    private static function storeTempFile($item, $source, $target) {
